...
def _define_tilelang_kernels():
    """Define tilelang kernels. Only called on CUDA platforms."""
    ...
    def mhc_pre_big_fuse_tilelang(
        ...
        """Fused kernels for mHC pre block (everything except gemm & sqrsum)."""
    ...
    return mhc_pre_big_fuse_tilelang, mhc_post_tilelang


# Lazily initialize tilelang kernels
_tilelang_kernels: tuple | None = None


def _get_tilelang_kernels():
    global _tilelang_kernels
    if _tilelang_kernels is None:
        if not current_platform.is_cuda_alike():
            raise RuntimeError("MHC kernels require CUDA")
        _tilelang_kernels = _define_tilelang_kernels()
    return _tilelang_kernels
...
def mhc_pre(
    ...
    mhc_pre_big_fuse_tilelang, _ = _get_tilelang_kernels()
...
def mhc_post(
    ...
    _, mhc_post_tilelang = _get_tilelang_kernels()
